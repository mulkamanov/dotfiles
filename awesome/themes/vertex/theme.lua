--[[
     ...
     Awesome WM theme
     based on Vertex theme from github.com/lcpz
...
local keyboardbg = wibox.container.background(mykeyboardlayout, beautiful.bg_normal, gears.shape.rectangle)
local keyboardwidget = wibox.container.margin(keyboardbg, 0, 0, 0, 0)
...
theme.cpu.widget.font = theme.font
...
theme.mem.widget.font = theme.font
...
theme.temp = lain.widget.temp {
    timeout = 10,
    settings = function()
        widget:set_markup("t: " .. coretemp_now .. " C°")
    end
}
theme.temp.widget.font = theme.font
...
-- browser_button
local browser_button = awful.widget.button({ image = "/usr/share/icons/hicolor/16x16/apps/chromium.png" })
browser_button:buttons(awful.util.table.join(
  awful.button({ }, 1, function () awful.spawn(awful.util.browser) end)
))

-- guieditor_button
local guieditor_button = awful.widget.button({ image = "/usr/share/icons/hicolor/16x16/apps/sublime-text.png" })
guieditor_button:buttons(awful.util.table.join(
  awful.button({ }, 1, function () awful.spawn("subl") end)
))
...
function theme.vertical_wibox(s)
    ...
    s.myleftwibox:setup {
        layout = wibox.layout.align.vertical,
        {
            ...
            s.mytaglist,
            lspace2,
            wibox.container.margin(browser_button, 5, 8, 13, 0),
            wibox.container.margin(guieditor_button, 5, 8, 13, 0),
    ...
    s.docktimer:connect_signal("timeout", function()
        ...
        guieditor_button.visible = false
